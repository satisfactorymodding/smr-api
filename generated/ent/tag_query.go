...
func (tq *TagQuery) QueryMods() *ModQuery {
...
func (tq *TagQuery) QueryGuides() *GuideQuery {
	query := (&GuideClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TagQuery) QueryGuideTags() *GuideTagQuery {
	query := (&GuideTagClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TagQuery) WithMods(opts ...func(*ModQuery)) *TagQuery {
	query := (&ModClient{config: tq.config}).Query()
...
func (tq *TagQuery) WithGuides(opts ...func(*GuideQuery)) *TagQuery {
	query := (&GuideClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) WithGuideTags(opts ...func(*GuideTagQuery)) *TagQuery {
	query := (&GuideTagClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withMods; query != nil {
		if err := tq.loadMods(ctx, query, nodes,
			func(n *Tag) { n.Edges.Mods = []*Mod{} },
			func(n *Tag, e *Mod) { n.Edges.Mods = append(n.Edges.Mods, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withGuides; query != nil {
		if err := tq.loadGuides(ctx, query, nodes,
			func(n *Tag) { n.Edges.Guides = []*Guide{} },
			func(n *Tag, e *Guide) { n.Edges.Guides = append(n.Edges.Guides, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withModTags; query != nil {
		if err := tq.loadModTags(ctx, query, nodes,
			...
			func(n *Tag, e *ModTag) { n.Edges.ModTags = append(n.Edges.ModTags, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withGuideTags; query != nil {
...
func (tq *TagQuery) loadMods(ctx context.Context, query *ModQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Mod)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ModsTable)
		s.Join(joinT).On(s.C(mod.FieldID), joinT.C(tag.ModsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.ModsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ModsPrimaryKey[1]))
...
func (tq *TagQuery) loadGuides(ctx context.Context, query *GuideQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Guide)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Tag)
	nids := make(map[string]map[*Tag]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.GuidesTable)
		s.Join(joinT).On(s.C(guide.FieldID), joinT.C(tag.GuidesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.GuidesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.GuidesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Guide](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "guides" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (tq *TagQuery) loadModTags(ctx context.Context, query *ModTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *ModTag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(modtag.FieldTagID)
	}
	query.Where(predicate.ModTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.ModTagsColumn), fks...))
	}))
...
func (tq *TagQuery) loadGuideTags(ctx context.Context, query *GuideTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *GuideTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Tag)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(guidetag.FieldTagID)
	}
