...
func (l *UserLoader) LoadThunk(key string) func() (*postgres.User, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*postgres.User, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *UserLoader) LoadAll(keys []string) ([]*postgres.User, []error) {
	results := make([]func() (*postgres.User, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	users := make([]*postgres.User, len(keys))
...
func (l *UserLoader) unsafeSet(key string, value *postgres.User) {
	if l.cache == nil {
		l.cache = map[string]*postgres.User{}
	}
	l.cache[key] = value
}
